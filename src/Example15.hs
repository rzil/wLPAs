...
phi :: WLPA.AtomType String String -> LaurentPolynomial (WLPA.Term String String Integer)
...
showTest = do
 ...
 where
  ...
  checks = map (all null . coefficients . p) check
  rels = WLPA.wLPA_relations_show weighted_graph_F
